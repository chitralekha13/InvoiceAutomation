...
def _extract_payment_details_from_igentic_response(resp: Dict) -> Optional[str]:
    ...
    def _search_in_text(raw: str) -> Optional[str]:
        if not raw or not isinstance(raw, str):
            return None
        raw_lower = raw.lower()
        # Payment agent outputs "payment summary" / "payment details" - be permissive
        ...
        if match:
            try:
                ...
                pass
        # Unlabeled ``` block with payment-related keys
        ...
        if match:
            try:
                ...
                pass
        # Any ``` ... ``` block containing JSON with payment keys
    ...
    # Use normalized iGentic structure (handles responseData, orchestration_result)
    data = _get_igentic_searchable(resp)
    raw = data.get("result") or data.get("display_text") or data.get("displayText") or ""
    if isinstance(raw, dict):
        # If result is already a dict with payment-like keys, use it
    ...
    if out:
        return out
    # Search in agentResponses (payment agent output may be there)
    ...
    if isinstance(agent_responses, list):
        for item in agent_responses:
            ...
            if out:
                return out
    # Fallback: search top-level response

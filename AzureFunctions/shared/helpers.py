...
# ============================================================================
# Microsoft Graph (SharePoint files) — replaces SharePoint REST / office365 SDK
# ============================================================================

_GRAPH_TOKEN_CACHE: Dict[str, Any] = {}
_GRAPH_SITE_ID_CACHE: Optional[str] = None
_GRAPH_DRIVE_ID_CACHE: Dict[str, str] = {}


def _decode_jwt_payload_without_verify(token: str) -> Dict[str, Any]:
    """Best-effort decode of JWT payload for diagnostics only (no signature verification)."""
    try:
...
def _acquire_graph_token() -> str:
    """Client-credentials token for https://graph.microsoft.com/.default (cert or secret)."""
    site_url = os.environ.get("SHAREPOINT_SITE_URL") or ""
    client_id = os.environ.get("AZURE_CLIENT_ID")
    if not client_id:
        raise ValueError("AZURE_CLIENT_ID is required for Microsoft Graph")

    tenant = (os.environ.get("AZURE_TENANT_ID") or "").strip()
    if not tenant:
        tenant = _get_sharepoint_tenant(site_url)
    authority = f"https://login.microsoftonline.com/{tenant}"
    using_cert = bool(os.environ.get("SHAREPOINT_CERT_BASE64") and os.environ.get("SHAREPOINT_CERT_THUMBPRINT"))
    # Avoid logging secrets; these are safe identifiers for diagnostics.
    logger.info(
        "Graph token acquire: client_id=%s tenant=%s auth_method=%s",
        client_id,
        tenant,
        "cert" if using_cert else "secret",
    )

    now = time.time()
    tok = _GRAPH_TOKEN_CACHE.get("access_token")
    exp = float(_GRAPH_TOKEN_CACHE.get("expires_at", 0))
    if tok and now < exp - 300:
        return tok

    cert_b64 = os.environ.get("SHAREPOINT_CERT_BASE64")
    cert_thumb = os.environ.get("SHAREPOINT_CERT_THUMBPRINT")
    if cert_b64 and cert_thumb:
        try:
            pem_str = base64.b64decode(cert_b64).decode("utf-8")
        ...
        app = msal.ConfidentialClientApplication(
            ...
            authority=authority,
            client_credential={
                "thumbprint": cert_thumb.strip().replace(" ", ""),
                "private_key": pem_str,
            },
        )
    else:
        ...
        if not secret:
            raise ValueError(
                "Graph auth needs SHAREPOINT_CERT_BASE64 + SHAREPOINT_CERT_THUMBPRINT "
                "or AZURE_CLIENT_SECRET on the same app registration."
            )
        app = msal.ConfidentialClientApplication(
            client_id,
            authority=authority,
    ...
    if "access_token" not in result:
        ...
        raise ValueError(f"Microsoft Graph token failed: {err}")

    claims = _decode_jwt_payload_without_verify(result["access_token"])
    logger.info(
        "Graph token claims: aud=%s tid=%s appid=%s roles=%s",
        claims.get("aud"),
        claims.get("tid"),
        claims.get("appid"),
        ",".join(claims.get("roles", [])) if isinstance(claims.get("roles"), list) else claims.get("roles"),
    )

    roles = claims.get("roles")
    if not roles:
        raise ValueError(
            "Microsoft Graph token has no application roles (roles claim missing/empty). "
            "Fix Entra app registration: add Microsoft Graph *Application* permissions "
            "(e.g. Sites.ReadWrite.All, Files.ReadWrite.All) and grant Admin consent "
            "in the same tenant as the SharePoint site."
        )

    _GRAPH_TOKEN_CACHE["access_token"] = result["access_token"]
    _GRAPH_TOKEN_CACHE["expires_at"] = now + float(result.get("expires_in", 3599))
    return result["access_token"]


def _graph_site_hostname_and_path() -> Tuple[str, str]:
    site_url = (os.environ.get("SHAREPOINT_SITE_URL") or "").strip()
    if not site_url:
    ...
    if not host or not path:
        raise ValueError("SHAREPOINT_SITE_URL must include hostname and site path (e.g. /sites/YourSite)")
    return host, path


def _get_graph_site_id() -> str:
    global _GRAPH_SITE_ID_CACHE
    if _GRAPH_SITE_ID_CACHE:
        return _GRAPH_SITE_ID_CACHE

    # Optional bypass: set to the Graph site id (guid,guid,guid) from SharePoint / Graph Explorer.
    env_site_id = (os.environ.get("SHAREPOINT_GRAPH_SITE_ID") or "").strip()
    if env_site_id:
        _GRAPH_SITE_ID_CACHE = env_site_id
        logger.info("Using SHAREPOINT_GRAPH_SITE_ID from environment (skip hostname path lookup)")
        return _GRAPH_SITE_ID_CACHE

    host, path = _graph_site_hostname_and_path()
    # Graph expects ONE path segment: "{hostname}:{serverRelativePath}" with server-relative path
    # starting at /sites/... — raw slashes break routing and often yield 401 generalException.
    server_rel = f"/{path}" if not path.startswith("/") else path
    site_identifier = f"{host}:{server_rel}"
    encoded = quote(site_identifier, safe="")
    url = f"https://graph.microsoft.com/v1.0/sites/{encoded}"
    logger.info("Graph site lookup: %s", site_identifier)

    token = _acquire_graph_token()
    r = requests.get(url, headers={"Authorization": f"Bearer {token}"}, timeout=60)
    if r.status_code == 401:
        # If a cached token is stale/invalid, re-acquire once and retry.
    ...
    if r.status_code != 200:
        raise ValueError(
            f"Graph site resolve failed {r.status_code} for site_identifier={site_identifier!r}: {r.text[:800]}"
        )
    _GRAPH_SITE_ID_CACHE = r.json()["id"]
    logger.info("Graph site id resolved for %s", host)
    return _GRAPH_SITE_ID_CACHE


def _get_graph_drive_id(site_id: str, library_name: str) -> str:
    key = f"{site_id}::{library_name}"
    if key in _GRAPH_DRIVE_ID_CACHE:
        return _GRAPH_DRIVE_ID_CACHE[key]
    token = _acquire_graph_token()
    ...
    try:
        ...
        raise ValueError(
            f"Graph drive list failed {r.status_code} for site_id={site_id}: {r.text[:800]}"
        ) from e
    for d in r.json().get("value", []):
        if (d.get("name") or "").strip() == library_name.strip():
            _GRAPH_DRIVE_ID_CACHE[key] = d["id"]
            return d["id"]
    raise ValueError(
        f"No document library named '{library_name}' on this site. "
        "Create it in SharePoint or fix the path (Graph lists drives by display name)."
    )


def _quote_graph_item_path(p: str) -> str:
    return quote(p.replace("\\", "/").strip("/"), safe="/")


def _graph_upload_item(drive_id: str, relative_path: str, content: bytes) -> Dict[str, Any]:
    """Upload file; relative_path is path under library root including filename."""
    enc = _quote_graph_item_path(relative_path)
    ...
    for attempt in range(max_attempts):
        ...
        if r.status_code in (200, 201):
            return r.json()
        # Transient Graph / service errors
        if r.status_code in (429, 500, 502, 503, 504) and attempt < max_attempts - 1:
            ...
            wait = min(wait, 60.0)
            logger.warning(
                "Graph upload retry %s/%s after HTTP %s (sleep %.1fs)",
                attempt + 1,
                max_attempts,
                r.status_code,
                wait,
    ...
    r = last_r
    raise RuntimeError(
        f"Graph upload failed {r.status_code}: {r.text[:1200]}"
    )


def _graph_download_item(drive_id: str, relative_path: str) -> bytes:
    token = _acquire_graph_token()
    enc = _quote_graph_item_path(relative_path)
    url = f"https://graph.microsoft.com/v1.0/drives/{drive_id}/root:/{enc}:/content"
    ...
    if r.status_code == 401:
        ...
        r = requests.get(url, headers={"Authorization": f"Bearer {token}"}, timeout=180)
    if r.status_code != 200:
        raise RuntimeError(f"Graph download failed {r.status_code}: {r.text[:1200]}")
    return r.content


def _split_tail_into_library_and_item(tail: str) -> Tuple[str, str]:
    tail = tail.strip("/").replace("\\", "/")
    if not tail:
        raise ValueError("Empty path under site")
    parts = tail.split("/", 1)
    lib = parts[0]
    if len(parts) < 2 or not parts[1]:
        raise ValueError(f"Need library/file path, got: {tail}")
    return lib, parts[1]


def _library_and_item_from_server_path(server_path: str) -> Tuple[str, str]:
    """Map /sites/Site/Lib/folder/file to (Lib, folder/file)."""
    prefix = _sharepoint_site_server_relative_prefix().rstrip("/")
    p = (server_path or "").strip()
    if p.startswith("http"):
        p = urlparse(p).path
    if not p.startswith("/"):
        p = "/" + p
    if not prefix:
        raise ValueError("SHAREPOINT_SITE_URL must be set to resolve SharePoint paths")
    if not p.lower().startswith(prefix.lower()):
        raise ValueError(f"File path must be under site {prefix!r}, got {p!r}")
    tail = p[len(prefix) :].lstrip("/")
    return _split_tail_into_library_and_item(tail)


# ============================================================================
# SharePoint path helpers (URLs for UI; Graph uses library + item path)
# ============================================================================
...
def _normalize_server_relative_url(path: str) -> str:
    """
    Normalize a SharePoint path to a server-relative URL for Graph path resolution.
...
def upload_file_to_sharepoint(file_content: bytes, file_name: str, folder_path: str = "Invoices") -> str:
    """
    Upload file via Microsoft Graph (drive item).

    Args:
        file_content: Raw file bytes
        file_name: File name in the library
        folder_path: Library name, optionally with subfolders, e.g. 'Invoices/2025/11_November'

    Returns:
        Browser URL (webUrl) when available, for storing in pdf_url and opening from the portal.
    """
    ...
    site_id = _get_graph_site_id()
    drive_id = _get_graph_drive_id(site_id, list_name)
    rel = f"{sub}/{file_name}".strip("/") if sub else file_name
    item = _graph_upload_item(drive_id, rel, file_content)
    return (item.get("webUrl") or "").strip()


def download_file_from_sharepoint(file_path: str) -> bytes:
    """Download file bytes via Graph. file_path: server-relative (/sites/.../Lib/...) or full SharePoint https URL."""
    lib, item_rel = _library_and_item_from_server_path(file_path)
    site_id = _get_graph_site_id()
    drive_id = _get_graph_drive_id(site_id, lib)
    return _graph_download_item(drive_id, item_rel)
...
def _normalize_postgres_dsn(raw: str) -> Union[str, Dict[str, str]]:
    """
    Build a psycopg2/libpq-compatible DSN from common Azure / portal formats.

    Accepts:
    - postgresql:// or postgres:// URI
    - jdbc:postgresql://... (JDBC prefix stripped)
    - Semicolon-separated ADO/.NET style (Host=...;Database=...;Username=...;Password=...;...)
    - Native libpq keyword=value (space-separated), passed through after trim
    """
    s = (raw or "").strip()
    if s.startswith("\ufeff"):
        s = s.lstrip("\ufeff")
    while len(s) >= 2 and ((s[0] == s[-1] == '"') or (s[0] == s[-1] == "'")):
        s = s[1:-1].strip()
    if not s:
        raise ValueError("SQL_CONNECTION_STRING is empty")

    low = s.lower()
    if low.startswith("postgresql://") or low.startswith("postgres://"):
        return s
    if low.startswith("jdbc:postgresql:"):
        rest = s[len("jdbc:postgresql:"):].lstrip()
        if rest.startswith("//"):
            return "postgresql:" + rest
        return "postgresql://" + rest.lstrip("/")

    # Semicolon-separated (Azure portal / ADO-style)
    if ";" in s and "=" in s:
        ...
        for part in parts:
            ...
            val = val.strip()
            lk = key.lower()
            if lk in ("host", "server", "hostname"):
                out["host"] = val
            elif lk == "port":
                out["port"] = val
            elif lk in ("database", "dbname", "db"):
                out["dbname"] = val
            elif lk in ("username", "user id", "userid", "user name"):
                out["user"] = val
            elif lk == "password":
                out["password"] = val
            elif lk in ("ssl mode", "sslmode"):
                sm = val.replace(" ", "").lower()
                if sm in ("require", "required", "true", "yes"):
                    out["sslmode"] = "require"
                elif sm in ("disable", "false", "no"):
                    out["sslmode"] = "disable"
                else:
                    out["sslmode"] = sm
        if out:
...
def get_sql_connection(cursor_factory=None):
    """Get PostgreSQL database connection"""
    conn_str = os.environ.get("SQL_CONNECTION_STRING")
    if not conn_str:
        raise ValueError("SQL_CONNECTION_STRING not found in environment")
    dsn = _normalize_postgres_dsn(conn_str)
    if isinstance(dsn, dict):
        kw = {k: v for k, v in dsn.items() if v is not None and v != ""}
...
def find_duplicate_invoice(fields: Dict) -> Optional[str]:
    """
    Return existing invoice_id if a row matches the new upload on all populated key fields (same-to-same).
    Gate: requires invoice_number, or (vendor_name + invoice_amount + at least one of invoice_date / start / end).
    When the new upload includes invoice_date, start_date, end_date, resource_name, or hours, each must match the row.
    """
    def _d10(v: Any) -> Optional[str]:
        if v is None:
            return None
        s = str(v).strip()[:10]
        return s if s else None

    def _float_close(a: Any, b: Any, tol: float = 0.01) -> bool:
        try:
    ...
    has_any_date = any(
        _d10(fields.get(k)) for k in ("invoice_date", "start_date", "end_date", "pay_period_start", "pay_period_end")
    )
    # Require enough identity to avoid false positives
    if not inv_num and not (vendor and amount is not None and has_any_date):
        return None
    conn = get_sql_connection()
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        select_cols = (
            "invoice_id, invoice_number, vendor_name, invoice_amount, invoice_date, start_date, end_date, "
            "invoice_hours, resource_name"
        )
        if inv_num:
            cursor.execute(
                f"SELECT {select_cols} FROM invoices WHERE LOWER(TRIM(invoice_number)) = LOWER(%s)",
                (inv_num.strip(),),
            )
        else:
            cursor.execute(f"SELECT {select_cols} FROM invoices")
        rows = cursor.fetchall()
        new_inv_dt = _d10(fields.get("invoice_date"))
        new_start = _d10(fields.get("start_date") or fields.get("pay_period_start"))
        new_end = _d10(fields.get("end_date") or fields.get("pay_period_end"))
        new_res = (str(fields.get("resource_name") or "").strip().lower())
        ...
        for row in rows:
            ...
            if amount is not None:
                if not _float_close(r_amount, amount):
                    continue
            # Same-to-same: each date field present on the new upload must equal the stored row
            if new_inv_dt:
                if new_inv_dt != _d10(row.get("invoice_date")):
                    continue
            if new_start:
                if new_start != _d10(row.get("start_date")):
                    continue
            if new_end:
                if new_end != _d10(row.get("end_date")):
                    continue
            if new_res:
                rr = (str(row.get("resource_name") or "").strip().lower())
                if rr != new_res:
                    continue
            if hours is not None and str(hours).strip() != "":
                if not _float_close(r_hours, hours):
                    continue
...
def _vendor_grouping_key(value: Optional[str]) -> str:
    """Normalize vendor names so minor variants (Inc vs Inc., spacing, case) group together."""
    s = _normalize_for_sow_match(value)
    if not s:
        return ""
    # "Convex Tech Inc." and "ConvexTech Inc" -> same key
    return re.sub(r"\s+", "", s)


def get_all_vendors() -> list:
    """Distinct vendors grouped by trivial spelling variants; one canonical label per group."""
    conn = get_sql_connection()
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    ...
    try:
        cursor.execute(
            """
            SELECT vendor_name, COUNT(*)::int AS cnt
            FROM invoices
        ...
        for row in rows:
            vn = dict(row)["vendor_name"]
            cnt = dict(row)["cnt"]
            groups[_vendor_grouping_key(vn)].append((vn, cnt))
        canonical: list[str] = []
        for _key, members in groups.items():
            members.sort(key=lambda x: (-x[1], len(x[0]), x[0].lower()))
            canonical.append(members[0][0])
        return sorted(set(canonical), key=lambda s: s.lower())
    finally:
        cursor.close()
        conn.close()

def _vendor_name_variants(cursor, vendor_name: str) -> list:
    cursor.execute(
        """
    ...
    names = [dict(r)["vendor_name"] for r in cursor.fetchall()]
    gk = _vendor_grouping_key(vendor_name)
    return [n for n in names if _vendor_grouping_key(n) == gk]


def get_vendor_resources(vendor_name: str):
    """Get all unique resource names for a vendor (all DB spellings grouped with vendor_name)."""
    if not os.environ.get('SQL_CONNECTION_STRING'):
        return []
    conn = get_sql_connection()
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        variants = _vendor_name_variants(cursor, vendor_name)
        if not variants:
            return []
...
def get_invoices_by_vendor_and_resources(vendor_name: str, resources: list= None):
    """Filter by vendor and resources from PostgreSQL (vendor matches all grouped name spellings)."""
    conn = get_sql_connection()
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    ...
    try:
        ...
        if not variants:
            return []
        vph = ",".join(["%s"] * len(variants))
        if resources and len(resources) > 0:
            placeholders = ','.join(['%s'] * len(resources))
            cursor.execute(f"""
                SELECT * FROM invoices
                WHERE vendor_name IN ({vph})
...
def _parse_date_to_date(value) -> Optional[date]:
    """Parse invoice/SOW date (string or date) to date for comparison."""
    ...
    # ISO-ish with time, e.g. 2026-03-01T00:00:00 or 2026-03-01 00:00:00
    try:
        return _dt.fromisoformat(s.replace("Z", "")).date()
    except Exception:
        pass

    # Extract date token from mixed text and parse.
    candidates = []
    m_iso = re.search(r"\b\d{4}[-/]\d{1,2}[-/]\d{1,2}\b", s)
    if m_iso:
        candidates.append(m_iso.group(0))
    m_num = re.search(r"\b\d{1,2}[-/]\d{1,2}[-/]\d{4}\b", s)
    if m_num:
        candidates.append(m_num.group(0))
    m_month = re.search(r"\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[a-z]*\s+\d{1,2},\s+\d{4}\b", s, re.I)
    if m_month:
        candidates.append(m_month.group(0))
    for token in candidates:
        for fmt in formats:
            try:
                return _dt.strptime(token, fmt).date()
            except ValueError:
                continue
    return None


def propagate_employee_id_to_matching_invoices(
    resource_name: Optional[str],
    consultancy_name: Optional[str],
    employee_id: Optional[str],
) -> int:
    """Mirror SOW employee_id to invoices with the same resource + vendor (consultancy)."""
    ...
    try:
        cursor.execute(
            """
            SELECT invoice_id, resource_name, vendor_name
            FROM invoices
            """
        )
        for row in cursor.fetchall():
            if _normalize_for_sow_match(row.get("resource_name")) != rn:
                continue
            if _normalize_for_sow_match(row.get("vendor_name")) != cn:
                continue
            cursor.execute(
                "UPDATE invoices SET employee_id = %s WHERE invoice_id = %s",
                (eid, row["invoice_id"]),
            )
        ...
        if updated:
            logger.info(
                "Propagated employee_id to %s invoice(s) for resource=%s consultancy=%s",
                updated,
                resource_name,
                consultancy_name,
            )
...
def get_sow_by_id(sow_id: str) -> Optional[Dict]:
    """Load one SOW row by sow_id."""
    conn = get_sql_connection()
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute(
            ...
            FROM sow_documents WHERE sow_id = %s
            """,
            (sow_id,),
        )
        row = cursor.fetchone()
...
def apply_manual_invoice_status_side_effects(
    invoice_id: str,
    existing: Dict,
    new_status: str,
    kwargs: Dict,
    ...
    """
    When the dashboard sets approval_status manually, apply the same side effects as automated flows:
    Approved → iGentic payment details; Payment Initiated → bill_pay_initiated_on;
    Pending / Need Approval / Invalid → clear payment_details (and bill pay where appropriate).
    """
    ...
    kwargs["approval_status"] = st
    kwargs["status"] = st

    if key == "approved":
        ah = existing.get("approved_hours")
        if ah is None:
            ah = existing.get("invoice_hours")
        try:
            ah_f = float(ah) if ah is not None else None
        except (TypeError, ValueError):
            ah_f = None
        label = (
            f"The Approved hours is {ah_f}. Get payment details"
            if ah_f is not None
            else "Get payment details for this invoice"
        )
        try:
            ...
            pd = _extract_payment_details_from_igentic_response(ok_result)
            if pd:
                kwargs["payment_details"] = pd
        except Exception as ex:
            logger.warning("Manual status Approved: iGentic payment details failed: %s", ex)

    elif key == "need_approval":
        kwargs["payment_details"] = None

    elif key == "pending":
        kwargs["payment_details"] = None
        kwargs["bill_pay_initiated_on"] = None

    elif key == "payment_initiated":
        kwargs["bill_pay_initiated_on"] = datetime.now(_tz.utc)
...
def insert_sow(
    ...
    sql = (
        f"INSERT INTO sow_documents ({', '.join(columns)}, created_at, last_updated_at) "
        f"VALUES ({ph}, NOW(), NOW())"
    )
    try:
...
def get_matching_sow(resource_name: Optional[str], consultancy_name: Optional[str]) -> Optional[Dict]:
    """
    Find a SOW that matches the given resource name only (consultancy_name is ignored).
    Uses _normalize_for_sow_match on resource_name, then performs a fuzzy best-match across likely candidates
    to handle minor name variations . If multiple SOWs are close matches,
    returns the best-scoring row (most recently updated on ties). Returns None if resource_name is empty
    after normalize or no candidate clears the similarity threshold.
    """
    def _tokens(s: str) -> list:
        import re as _re
        parts = [_re.sub(r"[^a-z0-9]+", "", p) for p in (s or "").split()]
        return [p for p in parts if p]

    def _core_tokens(s: str) -> list:
        # Ignore one-letter initials/noise (e.g. "G S") so "Jeethesh Reddy" matches "Jeethesh Reddy G S".
        return [t for t in _tokens(s) if len(t) > 1]

    def _token_overlap_score(a: str, b: str) -> float:
        ta = set(_core_tokens(a))
        tb = set(_core_tokens(b))
        if not ta or not tb:
            return 0.0
        inter = len(ta.intersection(tb))
        # Jaccard overlap
        return inter / float(len(ta.union(tb)))

    def _token_similar(a: str, b: str) -> bool:
        # Exact match is always accepted.
        if a == b:
            return True
        # Do not allow very short fragments such as "jee" ~= "jeethesh".
        if len(a) < 4 or len(b) < 4:
            return False
        from difflib import SequenceMatcher
        # Allow small spelling mistakes like "jayanti" vs "jayanthi".
        return SequenceMatcher(None, a, b).ratio() >= 0.84

    def _name_gate_match(invoice_name: str, sow_name: str) -> bool:
        """
        Human-like rule:
        - compare meaningful name tokens (ignore single-letter initials),
        - treat minor spelling mistakes as same token,
        - require at least two matched tokens in any combination
          (first+last, first+middle, middle+last, etc.),
        - short partials are not treated as matches.
        """
        inv_t = _core_tokens(invoice_name)
        sow_t = _core_tokens(sow_name)
        if not inv_t or not sow_t:
            return False
        used_sow_idx = set()
        match_count = 0
        for a in inv_t:
            for i, b in enumerate(sow_t):
                if i in used_sow_idx:
                    continue
    ...
    def _similarity(a: str, b: str) -> float:
        # Blend token overlap and sequence similarity (helps with missing/extra middle names).
        from difflib import SequenceMatcher
        a = a or ""
        b = b or ""
        seq = SequenceMatcher(None, a, b).ratio()
        tok = _token_overlap_score(a, b)
        return 0.60 * seq + 0.40 * tok

    rn = _normalize_for_sow_match(resource_name)
    if not rn:
        return None
    rn_tokens = _tokens(rn)
    rn_first = rn_tokens[0] if rn_tokens else ""
    conn = get_sql_connection()
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        # Pull likely candidates:
        # - If we have a first token, use ILIKE to reduce scan size.
        # - Always order by last_updated_at so tie-break is deterministic.
        if rn_first:
            cursor.execute("""
                SELECT sow_id, doc_name, pdf_url, resource_name, consultancy_name,
                       sow_start_date, sow_end_date, net_terms, max_sow_hours, rate_per_hour,
                       project_role, sow_project_duration, employee_id, last_updated_at
                FROM sow_documents
                WHERE COALESCE(resource_name, '') ILIKE %s
        ...
        if best_row is None:
            # Fuzzy match with explicit gate:
            # first+last OR first+middle must match (with minor spelling tolerance).
            threshold = 0.66
            for row in rows:
                sow_rn = _normalize_for_sow_match(row.get("resource_name"))
                if not sow_rn:
                    continue
                if not _name_gate_match(rn, sow_rn):
                    continue
                score = _similarity(rn, sow_rn)
                if score > best_score:
                    best_score = score
                    best_row = row
        ...
        for k, v in d.items():
            if hasattr(v, "isoformat"):
                d[k] = v.isoformat()
        # last_updated_at is used only for tie-break; don't leak unless caller expects it
...
def merge_sow_into_invoice_fields(
    ...
    - Copies employee_id from SOW when set (canonical source); timesheet only fills invoices that still lack it.
    ...
    # Employee ID: SOW is the canonical source when present (entered on SOW dashboard).
    sow_eid = (sow.get("employee_id") or "").strip()
    if sow_eid:
        fields["employee_id"] = sow_eid
        ...
        try:
            ...
            for _ in range(60):
                ...
                if result.get("status") == "succeeded":
                    ...
                    logger.info(
                        "Document Intelligence pages returned for %s: %s",
                        filename,
                        len(pages),
                    )
                    ...
                    # Allow larger payloads when needed (e.g., SOW parsing). Keep a safety cap to avoid huge responses.
                    try:
                        full_text_limit = int(os.environ.get("DI_FULL_TEXT_LIMIT", "200000") or "200000")
                    except Exception:
                        full_text_limit = 200000
                    if full_text_limit <= 0:
                        full_text_limit = 200000
                    ...
                    return {
                        ...
                        "full_text": (full_text[:full_text_limit] if full_text else ""),
...
def get_cached_timesheet_fields_for_invoice(
    employee_id: Optional[str],
    resource_name: Optional[str],
    ...
    """
    Look back into timesheet_hours_cache and return cached timesheet fields for invoice month.
    Month is derived strictly from pay period bounds (start_date/end_date), not invoice_date.
    Priority:
    1) employee_id + month exact
    2) closest resource_name within month (only when employee_id is unavailable/missing)
    """
    from difflib import SequenceMatcher

    def _ym_candidates_from_dates() -> list:
        # Strict rule: sync by pay-period bounds only (start/end), never invoice_date.
        out = []
        sd = _parse_date_to_date(start_date)
        ed = _parse_date_to_date(end_date)
        for d in (sd, ed):
            if not d:
                continue
            ym = (d.year, d.month)
    ...
    def _row_payload(row: Any) -> Dict[str, Any]:
        if not row:
            return {"approved_hours": None, "division": None, "client_name": None, "project_name_excel": None}
        return {
            "approved_hours": _as_float(row.get("approved_hours")),
            "division": row.get("division"),
            "client_name": row.get("client_name"),
            "project_name_excel": row.get("project_name_excel"),
        }

    ym_candidates = _ym_candidates_from_dates()
    ...
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        for year, month in ym_candidates:
            emp = (employee_id or "").strip()
            if emp:
                try:
                    cursor.execute("""
                        SELECT approved_hours, division, client_name, project_name_excel
                        FROM timesheet_hours_cache
                    ...
                    # Older schema without dimension columns.
                    cursor.execute("""
            ...
            try:
                cursor.execute("""
                    SELECT resource_name, approved_hours, division, client_name, project_name_excel, updated_at
                    FROM timesheet_hours_cache
                    WHERE year = %s AND month = %s
                ...
                # Older schema without dimension columns.
                cursor.execute("""
                    SELECT resource_name, approved_hours, updated_at
                    FROM timesheet_hours_cache
                    WHERE year = %s AND month = %s
                      AND COALESCE(resource_name, '') <> ''
                    ORDER BY updated_at DESC
            ...
            best_score = 0.0
            for row in rows:
                cand = _normalize_for_sow_match(row.get("resource_name"))
                if not cand:
                    continue
                score = SequenceMatcher(None, rn, cand).ratio()
                if score > best_score:
                    best_score = score
                    best = row
            if best and best_score >= 0.72:
                return _row_payload(best)
        return None
    except Exception:
        # Cache table may not exist in older envs; fail soft.
        return None
    finally:
        cursor.close()
        conn.close()
...
def get_cached_timesheet_approved_hours_for_invoice(
    employee_id: Optional[str],
    resource_name: Optional[str],
    invoice_date: Optional[str] = None,
    start_date: Optional[str] = None,
    ...
    cached = get_cached_timesheet_fields_for_invoice(
    ...
    if not cached:
        return None
    return cached.get("approved_hours")


def _timesheet_sp_folder_path() -> str:
    return (os.environ.get("SP_FOLDER_PATH") or "Timesheet").strip().strip("/").replace("\\", "/")


def timesheet_canonical_filename(year: int, month: int, ext: str = "xlsx") -> str:
    """Same basename sync-excel uses when the workbook has one unambiguous month."""
    return f"timesheet_{year}_{month:02d}.{ext}"


def _server_relative_timesheet_file(year: int, month: int, ext: str) -> str:
    folder = _timesheet_sp_folder_path()
    name = timesheet_canonical_filename(year, month, ext)
    return _normalize_server_relative_url(f"{folder}/{name}")


def _parse_timesheet_workbook_rows(file_bytes: bytes) -> List[Dict[str, Any]]:
    """Same shape as sync-excel: header row lowercased, dict per data row."""
    import io
    import openpyxl
...
def _ts_get_col(row: dict, col_name: str) -> str:
    key = col_name.lower()
...
def _ts_month_tuple_from_row(row: dict) -> Tuple[Optional[int], Optional[int]]:
    val = _ts_get_col(row, "date")
    if not val:
        return None, None
...
def _ts_employee_id_column_e(row: dict) -> str:
    """Column E = zero-based index 4 in parsed row order (sync-excel)."""
    try:
...
def _ts_parse_hours_cell(val) -> float:
...
def get_timesheet_fields_from_sharepoint_for_pay_month(
    ...
    """
    Load the canonical monthly timesheet from SharePoint (timesheet_YYYY_MM.xlsx)
    for the invoice pay period month — derived from start_date/end_date only, not invoice_date.

    Used when DB timesheet_hours_cache has no row yet or is incomplete.
    """
    from difflib import SequenceMatcher

    d = _parse_date_to_date(start_date) or _parse_date_to_date(end_date)
    if not d:
        return None
    year, month = d.year, d.month

    if not (os.environ.get("SHAREPOINT_SITE_URL") or "").strip():
        return None

    file_bytes: Optional[bytes] = None
    for ext in ("xlsx", "xlsm"):
        try:
            rel = _server_relative_timesheet_file(year, month, ext)
            file_bytes = download_file_from_sharepoint(rel)
            if file_bytes:
                break
        except Exception as ex:
            logger.info(
                "SharePoint timesheet not found or unreadable (%s-%s .%s): %s",
                year,
                month,
                ext,
                ex,
    ...
    try:
        rows = _parse_timesheet_workbook_rows(file_bytes)
    except Exception as ex:
        logger.warning("Failed to parse SharePoint timesheet %s-%s: %s", year, month, ex)
        return None
    ...
    defaults = lambda: {
        ...
        "division": None,
        "client_name": None,
        "project_name_excel": None,
        "disp_name": "",
    }

    for row in rows:
        ry, rm = _ts_month_tuple_from_row(row)
        if ry != year or rm != month:
            continue
        eid = (_ts_employee_id_column_e(row) or "").strip().lower()
        fn = _ts_get_col(row, "first name")
        ln = _ts_get_col(row, "last name")
        disp = _normalize_for_sow_match(f"{fn} {ln}".strip())
        key = f"emp:{eid}" if eid else f"name:{disp or 'unknown'}"
        if key not in buckets:
            buckets[key] = defaults()
        b = buckets[key]
        if not b["disp_name"] and disp:
            b["disp_name"] = disp
        appr = (_ts_get_col(row, "approval status") or "").strip().lower()
        if appr == "approved":
            hrs = _ts_get_col(row, "hour(s)") or _ts_get_col(row, "hours") or "0"
            b["hours"] += _ts_parse_hours_cell(hrs)
        for dim_key, col in (
            ("division", "division"),
            ("client_name", "client name"),
            ("project_name_excel", "project name"),
        ):
            if b[dim_key] in (None, ""):
                v = _ts_get_col(row, col)
                if v:
                    b[dim_key] = v
    ...
    emp = (employee_id or "").strip().lower()
    if emp:
        hit = buckets.get(f"emp:{emp}")
        if hit is None:
            return None
        return {
            "approved_hours": hit["hours"] if hit["hours"] else None,
            "division": hit["division"],
            "client_name": hit["client_name"],
            "project_name_excel": hit["project_name_excel"],
        }

    rn = _normalize_for_sow_match(resource_name or "")
    if not rn:
        return None
    best = None
    best_score = 0.0
    ...
    if best and best_score >= 0.72:
        return {
            "approved_hours": best["hours"] if best["hours"] else None,
            "division": best["division"],
            "client_name": best["client_name"],
            "project_name_excel": best["project_name_excel"],
        }
    return None
...
def update_excel_file(invoice_id: str, invoice_data: Dict) -> None:
    ...
    # Save and upload back via Microsoft Graph
    output = io.BytesIO()
    wb.save(output)
    output.seek(0)
    lib, item_rel = _library_and_item_from_server_path(server_relative_excel_url)
    site_id = _get_graph_site_id()
    drive_id = _get_graph_drive_id(site_id, lib)
    _graph_upload_item(drive_id, item_rel, output.getvalue())
...
def upload_excel_to_sharepoint(
    ...
    """
    Upload an Excel timesheet file via Microsoft Graph (same as invoice PDF upload).

    Args:
        file_content : Raw bytes of the .xlsx file.
        file_name    : Filename (special chars are sanitised).
        folder_path  : Library name, optionally with subfolders, e.g. "Timesheet/2025/05_May".
    """
    safe_name = re.sub(r'[\\/:*?"<>|#%]', '_', file_name)
    return upload_file_to_sharepoint(file_content, safe_name, folder_path)
...
def refresh_monthly_sync_report_with_invoice_update(
    start_date: Optional[str],
    end_date: Optional[str],
    employee_id: Optional[str],
    resource_name: Optional[str],
    ...
    """
    Update existing monthly sync report row(s) after invoice upload so the same
    SharePoint report stays current without creating a new file.
    """
    d = _parse_date_to_date(start_date) or _parse_date_to_date(end_date)
    if not d:
        return False
    ...
    year, month = d.year, d.month
    report_name = f"sync_report_{year}_{month:02d}.xlsx"
    folder_path = f"{_timesheet_sp_folder_path()}/SyncReports"
    server_rel = _normalize_server_relative_url(f"{folder_path}/{report_name}")

    try:
        report_bytes = download_file_from_sharepoint(server_rel)
    except Exception as e:
        logger.info("Monthly sync report %s not found yet: %s", report_name, e)
        return False
    ...
    try:
        ...
        ws = wb["Unmatched"] if "Unmatched" in wb.sheetnames else wb.active

        eid = (employee_id or "").strip().lower()
        rn = _normalize_for_sow_match(resource_name or "")
        st = (approval_status or "").strip().lower()
        if st in ("approved", "complete", "ready for payment", "payment initiated"):
            report_status = "MATCHED"
        elif st in ("need approval", "need_approval", "need manual review"):
            report_status = "Need Approval"
        elif st == "pending":
            report_status = "PENDING"
        else:
            report_status = "UNMATCHED"

        changed = False
        for r in range(3, ws.max_row + 1):
            row_year = ws.cell(r, 4).value
            row_month = ws.cell(r, 5).value
            try:
            ...
            row_eid = str(ws.cell(r, 3).value or "").strip().lower()
            row_name = _normalize_for_sow_match(str(ws.cell(r, 2).value or ""))
            eid_match = bool(eid and row_eid and eid == row_eid)
            name_match = bool((not eid_match) and rn and row_name and rn in row_name)
            if not (eid_match or name_match):
                continue

            ws.cell(r, 1, value=report_status)
            if resource_name:
                ws.cell(r, 11, value=resource_name)
            if employee_id:
                ws.cell(r, 12, value=str(employee_id))
            changed = True
        ...
        wb.save(out)
        out.seek(0)
        upload_file_to_sharepoint(out.getvalue(), report_name, folder_path)
        return True
    except Exception as e:
        logger.warning("Failed to refresh monthly sync report %s: %s", report_name, e)
        return False

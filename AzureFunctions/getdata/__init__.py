...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    pdf_url = (body or {}).get("pdf_url")
    file_name = (body or {}).get("fileName")
    ...
    try:
        ...
        if action == "stream":
            # 1) Invoice flow: stream invoice doc by documentId (existing behavior)
            if document_id:
                inv = get_invoice(document_id)
                if not inv:
                    return func.HttpResponse(
                ...
                resolved_file_name = inv.get("doc_name") or "document.pdf"
                resolved_pdf_url = sharepoint_url
            # 2) SOW flow: stream by pdf_url directly (used by sow.html)
            else:
                resolved_pdf_url = pdf_url
                resolved_file_name = file_name or "document.pdf"

            if not resolved_pdf_url:
                return func.HttpResponse(
                    json.dumps({"error": "No file URL (provide documentId or pdf_url)"}),
                    status_code=400,
                    mimetype="application/json",
                )

            try:
                file_content = download_file_from_sharepoint(resolved_pdf_url)
            ...
            return func.HttpResponse(
                ...
                headers={
                    ...
                    "Content-Disposition": f'inline; filename="{resolved_file_name}"',

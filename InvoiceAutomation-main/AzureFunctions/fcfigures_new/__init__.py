...
# Handle other fields from dashboard edit (due_date, invoice_amount, etc.)
        ...
        # Handle approval_status from dashboard edit
        if "approval_status" in body:
            status_val = body["approval_status"]
            kwargs["approval_status"] = status_val
            kwargs["status"] = status_val
            logger.info("Updated approval_status and status: %s", status_val)
        ...
        if kwargs:
            ...
            if kwargs_clean:
                ...
                if kwargs_clean.get("employee_id"):
                    invp = get_invoice(invoice_id)
                    if invp:
                        propagate_employee_id_to_matching_sow(
                            invp.get("resource_name"),
                            invp.get("vendor_name"),
                            kwargs_clean.get("employee_id"),
                        )

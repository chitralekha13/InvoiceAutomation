...
from shared.helpers import upload_excel_to_sharepoint,upload_sync_report_to_sharepoint,_extract_payment_details_from_igentic_response,continue_igentic_session,propagate_employee_id_to_matching_sow
...
COL_EMPLOYEE_ALIASES = ('employee id', 'employee_id', 'emp id', 'employeeid', 'emp_id')
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        cursor.execute("""
            SELECT invoice_id, resource_name, vendor_name, start_date, end_date,
...
# Core processing 
def _first_employee_id(group: list):
    for alias in COL_EMPLOYEE_ALIASES:
        v = _first_val(group, alias)
        if v:
            return str(v).strip()
    return None


def _write_invoice_update(cursor, conn, inv: dict, fields: dict):
    _write_update(cursor, conn, inv['invoice_id'], fields)
    eid = fields.get('employee_id')
    if eid:
        propagate_employee_id_to_matching_sow(
...
def _process_group(first, last, yr, mo, group, invoices, cursor, conn) -> dict:
    ...
    ts_employee_id = _first_employee_id(group)
    ...
    if all_approved and match_status == 'MATCHED':
        ...
        _write_invoice_update(cursor, conn, inv, fields_ok)
    ...
    #Case 3: Both names matched + mixed approval (some approved, some not)
        ...
        _write_invoice_update(cursor, conn, inv, {
            'approval_status': 'Need Approval',
            **_dims_update(),
        })
        return {**base,
    ...
    # All still Pending — update dimensions only, leave approval_status
        total_hours = sum(_to_float((_get_col(r, COL_HOURS) or _get_col(r, 'hours') or '0').strip()) for r in group)
        _write_invoice_update(cursor, conn, inv, _dims_update())
        return {**base,
                'status':       'PENDING',
                'invoice_id': str(inv['invoice_id']),
                'matched_to':   inv.get('resource_name'),
...
def _generate_comparison_report(unmatched_results: list, all_results: list, db_invoices: list, source_filename: str, groups: dict) -> bytes:
    ...
    cols2 = ['Invoice ID', 'Resource Name', 'Vendor Name', 'Start Date', 'End Date',
             'Invoice Hours', 'Approval Status', 'Division', 'Client Name', 'Employee ID']
    ...
    for col, width in zip('ABCDEFGHIJ', [14, 28, 22, 13, 13, 13, 16, 18, 22, 14]):
    ...
    cols3 = ['Excel Name', 'Matched To (DB)', 'Invoice ID', 'Year', 'Month',
             'Approved Hours', 'Invoice Hours', 'New DB Status', 'Row Count', 'Employee ID (timesheet)']
    ...
    for r_idx, result in enumerate(matched_results, start=3):
        row_data = [
            ...
            result.get('employee_id') or '',
        ]
        for c_idx, val in enumerate(row_data, 1):
            style_cell(ws3.cell(row=r_idx, column=c_idx, value=val), 'EAFAF1')

    for col, width in zip('ABCDEFGHIJ', [28, 28, 14, 8, 8, 15, 13, 16, 10, 18]):
